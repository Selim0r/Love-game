...
export default function HeartDrag({ onUnlocked }) {
  ...
  function move(e) {
    if (!dragging) return;
    const rect = heartRef.current.parentElement.getBoundingClientRect();
    setPos({
  ...
  function endDrag() {
    ...
    if (overlap) {
      onUnlocked("You bring joy into my life.");
    } else {
      setPos({ x: 0, y: 0 });
    }
  }
  ...
  return (
    <div className="relative h-64 flex flex-col items-center justify-between">
      <p className="text-sm sm:text-base text-rose/80 mb-3">
        Heart কে বাক্সে টেনে আন।
      </p>

      <div className="relative w-full flex items-center justify-between px-4">
        <div
          ref={boxRef}
          className="w-36 h-36 sm:w-40 sm:h-40 lg:w-44 lg:h-44 rounded-xl border-2 border-dashed border-rose/60 
          flex items-center justify-center bg-white/60"
        >
          <span className="text-rose/80 text-sm sm:text-base">Love Box</span>
        </div>
        ...
        <div className="w-24 h-24 sm:w-28 sm:h-28 lg:w-32 lg:h-32 relative">
          <div
            ...
            className="w-full h-full bg-pink-400 rounded-full 
            flex items-center justify-center shadow-lg cursor-grab"
          >
